...
import {
  MessageCircle, Mail, Phone, MapPin, ShieldCheck, Gauge, Baby, Route,
  MapPinned, Star,
} from "lucide-react";
...
export function Footer({ locale, dict }: { locale: Locale; dict: Dict }) {
  ...
  const legalLinks = [
    { href: localePath(locale, "terms"), label: "Terms" },
    { href: `${SITE.domain}/sitemap.xml`, label: "Sitemap", external: true },
    { href: SITE.bookingUrl, label: dict.nav.book, external: true },
  ];
    ...
    <footer className="relative overflow-hidden border-t border-border bg-[#fff4e5] text-[var(--ink)] dark:border-white/10 dark:bg-[var(--ink)] dark:text-white">
      ...
      <div className="relative mx-auto max-w-7xl px-4 py-16 sm:px-6 lg:px-8">
        ...
        <div className="grid gap-10 md:grid-cols-2 lg:grid-cols-12">
          ...
          <FooterCol title={dict.footer.fleet} links={fleetLinks} className="lg:col-span-3" />
          <FooterCol title={dict.footer.company} links={infoLinks} className="lg:col-span-3" />
          <FooterCol title={dict.footer.explore} links={exploreLinks} className="lg:col-span-2" />
        </div>

        <div className="mt-12 flex flex-col gap-4 border-t border-border pt-8 md:flex-row md:items-center md:justify-between dark:border-white/10">
          ...
          <div className="flex items-center gap-4 text-sm text-muted-foreground">
            {legalLinks.map((it) => it.external ? (
              <a key={it.href} href={it.href} className="hover:text-[var(--sea)]" target="_blank" rel="noopener noreferrer">{it.label}</a>
            ) : (
              <Link key={it.href} href={it.href} className="hover:text-[var(--sea)]">{it.label}</Link>
            ))}
          </div>
        </div>
      </div>
...
function FooterCol({ title, links, className = "" }: { title: string; links: { href: string; label: string }[]; className?: string }) {
  return (
    <div className={className}>
      <h4 className="mb-3 text-sm font-bold text-[var(--ink)] dark:text-white">{title}</h4>
